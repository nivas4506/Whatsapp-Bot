...
export class MemoryContactRepository implements IContactRepository {
  ...
  async updateLastSeen(id: string): Promise<void> {
    ...
    if (contact) contact.lastSeenAt = new Date();
  }
}
...
export class MemoryRequirementRepository implements IRequirementRepository {
  private requirements: Map<string, RequirementRecord> = new Map();
  ...
  async findByReferenceId(referenceId: string): Promise<RequirementRecord | null> {
    for (const r of this.requirements.values()) {
      if (r.referenceId === referenceId) return r;
    }
    return null;
  }
  ...
  async updateStatus(
    ...
    if (!req) return null;
    req.status = status;
    if (reviewer) req.assignedReviewer = reviewer;
...
export class MemoryFAQRepository implements IFAQRepository {
  ...
  async findMatching(query: string, locale: string = 'en'): Promise<FAQEntry | null> {
    ...
    for (const entry of this.entries) {
      if (entry.status !== 'ACTIVE' || entry.locale !== locale) continue;
      for (const p of entry.questionPatterns) {
        if (normalized.includes(p.toLowerCase())) return entry;
      }
    }
    return null;
  }
...
export class MemoryFormConfigRepository implements IFormConfigRepository {
  private configs: Map<RequirementCategory, FormConfig> = new Map([
    [
      'CERTIFICATE',
      {
        id: 'cfg-cert',
