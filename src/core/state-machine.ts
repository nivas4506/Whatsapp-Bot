...
export class ConversationStateMachine {
  public static canTransition(current: ConversationState, target: ConversationState): boolean {
    if (current === target) return true;
    const allowed = VALID_TRANSITIONS[current];
    return allowed ? allowed.includes(target) : false;
  }

  public static assertValidTransition(current: ConversationState, target: ConversationState): void {
