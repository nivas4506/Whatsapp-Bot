...
export class ConversationOrchestrator {
    async processInboundMessage(inbound) {
        ...
        await repositories.messages.recordMessage(providerMessageId, conversation.id, 'INBOUND', type, text);
        ...
        const isGreeting = ['hi', 'hello', 'hey', 'start', 'help'].includes(text.toLowerCase().trim()) || isFirstTime;
        if (classification.isEscalated) {
            ...
            const req = await repositories.requirements.create(conversation.id, classification.category, text, formConfig.url, classification.urgency);
        ...
        await repositories.messages.recordMessage(outboundResult.messageId, conversation.id, 'OUTBOUND', ctaUrl ? 'interactive_cta' : interactiveOptions ? 'interactive_button' : 'text', replyText);
